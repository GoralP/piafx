...
class PriceChart extends PureComponent {
  ...
  componentWillMount() {
    ...
    document.addEventListener("keydown", function(event) {
      if (event.key === "Delete") {
        temp.setState({ deletePressed: true });
      }
      if (event.key === "Delete" && event.shiftKey) {
        temp.setState({ shiftDeletePressed: true });
      }
    });
  }
  ...
  render() {
    ...
    if (this.props.priceData.length > 0) {
      if (this.props.priceData[0].length > 0) {
        ...
        // firstPlot.title("helllllllllllllllllllllllloooooooooooooooooooooo");
        firstPlot
          .yAxis()
          .labels()
          .fontColor(charttext);
        ...
        if (this.state.deletePressed) {
          this.removeSelectedDrawing(chart);
        }
        if (this.state.shiftDeletePressed) {
          this.removeDrawing(chart);
        }
        var temp_this = this;
        // document.addEventListener("keydown", function(event) {
        //   console.log('Hii');
        //   if (event.key === "Delete" && selectedDrawing != '') {
        //       console.log('Hii1');
        //       temp_this.removeSelectedDrawing(chart)
        //   }
        //   if (event.key === "Delete" && event.shiftKey) {
        //     temp_this.removeDrawing(chart)
        //   }
        // });
            ...
            if ("data" in this.props.snrData) {
              if (this.props.snrData["data"].length > 0) {
                ...
                this.props.snrData["data"].forEach((element, key) => {
                  ...
                  // var color = 'grey';
                  if (
                    element["Lable"] == "Support" ||
                    element["Lable"] == "Resistance"
                  ) {
                    // trendline.normal().stroke(supportColor, 2, "10 2");
                    trendline.normal().stroke(color + " 0.7", 1);
                    color = color;
                  } else if (
                    element["Lable"] == "Avg. Support" ||
                    element["Lable"] == "Avg. Resistance"
                  ) {
                    trendline.normal().stroke(averagecolor, 3);
                    color = averagecolor;
                  }
              ...
              if (this.props.snrData["data"].length > 0) {
                ...
                this.props.snrData["data"].forEach((element, key) => {
                  ...
                  if (
                    ...
                    if (element["X1"] == verticalline_date) {
                      trendline.normal().stroke(color, 1);
                      trendline.hovered().stroke(color, 1);
                      trendline.selected().stroke(color, 2);
                    } else {
                      trendline.normal().stroke(color, 1, "1 4");
                      trendline.hovered().stroke(color, 1, "1 4");
                      trendline.selected().stroke(color, 1, "1 4");
                    }
                    color = color;
                  } else if (
                    element["Lable"] == "Avg. Support" ||
                    ...
                    if (element["X1"] == verticalline_date) {
                      trendline.normal().stroke(averagecolor, 3);
                      trendline.hovered().stroke(averagecolor, 4);
                      trendline.selected().stroke(averagecolor, 4.5);
                    } else {
                      trendline.normal().stroke(averagecolor, 3);
                      trendline.hovered().stroke(averagecolor, 4);
                      trendline.selected().stroke(averagecolor, 4.5);
                  ...
                  //     trendline.selected().stroke(resistanceAverageColor, 4.5);
                  //   }
                  //   else{
                  //     trendline.normal().stroke(resistanceAverageColor, 3);
                  //     trendline.hovered().stroke(resistanceAverageColor, 4);
              ...
              if (this.props.exitData[0] != "") {
                ...
                series.risingStroke(function() {
                  if (this.x) {
                    ...
                    //   tempcolor =  '#D546A6'
                    // }
                    // if (datesarray[pointDateString] && (greydate == pointDateString || greydate == pointDateString1))
                    // {
                    //   tempcolor =  '#C721F0'
                    // }
                    if (tempcolor != "") {
        ...
        firstPlot.legend().listen("legendItemClick", function(e) {
          ...
          // console.log("testseries", seriesArray);
        });
        if (seriesArray["price"]) {
        ...
        //   var cotreq = { id : ticker_id , date : pointDateString}
        //   this.props.mekkoData(cotreq);
        //   this.props.getgauge({'ticker_code':localStorage.getItem('ticker_code'),'date':pointDateString1,'william_period':this.props.gaugePeriod['william'],'slope_period':this.props.gaugePeriod['slope'],'nclra_period':this.props.gaugePeriod['nclra'] });
        //   this.setState({
        ...
        //   console.log('Axis Pointer Date: ' + pointDateString1 + ', Open: ' + open+ ', High: ' + high+ ', Low: ' + low+ ', Close: ' + close)
        //   // series.tooltip().format('Axis Pointer Date: ' + pointDateString + ', Open: ' + open+ ', High: ' + high+ ', Low: ' + low+ ', Close: ' + close);
        //   // }
        // });
        ...
        //     {
        //       var data1 = {symbol:this.state.symbol,date : pointDateString==''?this.state.statedate:pointDateString, price : value==''?this.state.statevalue:value,cot : '',ecin : '',description:this.state.description,format:this.state.format,datestamp:this.state.datestamp}
        ...
        if (this.props.graphdata != "" || cotdataArray.length > 0) {
          ...
          cotdataArray.map((firstkey, mainitem) => {
            ...
            // for legacy open interest chart
            ...
            Object.keys(firstkey[0]).map((item, key) => {
              ...
              //       change = true;
              //       console.log('aspl',this.props.savestatus1)
              //       if(!index_array.includes(firstkey['ciname']))
              //       {
              //         this.props.savestatus(false);
              //       }
        ...
        if (this.props.sapratetechnicalindicator.length > 0) {
          ...
          this.props.sapratetechnicalindicator.map((item, index) => {
            if (item != null) {
              ...
              //       if(select2.getColumn(-1*(index+1))=='' || typeof select2.getColumn(-1*(index+1)) === "undefined" || typeof select2.getColumn(-1*(index+1)) === undefined )
              //         {var value = '';}
              //       else
              //         {var value = select2.getColumn(-1*(index+1)).toFixed(2);}
              //         // item[firstkey['ciname']] = [];
        ...
        if (
          ...
          series_cot.stroke("2 green");
          if (theme == "light") {
            series_cot.stroke("2 MediumSpringGreen");
          }
          // series_cot.fill(palette[5]);
        ...
        if (this.props.ecinData["ecin_data"] || ecindataArray.length > 0) {
          ...
          ecindataArray.map((firstkey, mainitem) => {
            ...
            if (
              this.props.drawingArray[firstkey["ecin_details"][0]["short_desc"]]
            ) {
              ecinPlot
                .annotations()
                .fromJson(
          ...
          <div
            className={
              this.state.addvisible
                ? "cust_price_chart container_white SE"
                : "cust_price_chart SE"
            }
          >
            <p>
              ...
              >
                ...
                {/* <Popover overlayClassName="gx-popover-horizantal" placement="bottomRight" visible={this.state.visible} content={markerData} trigger="click"   onCancel={this.handleCancel}>
                  <span className="gx-pointer gx-flex-row gx-align-items-center markerIcon" onClick={()=>this.showModal()} style={{'marginTop':'5px'}} title="Marker"></span> */}
