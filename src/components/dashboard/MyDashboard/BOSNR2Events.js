...
import { getSNR2Events } from "appRedux/actions/Exchange";
...
class BOSNR2Events extends Component {
  constructor(props) {
    super(props);
  }
  state = {
  ...
  render() {
    ...
    if (
      ...
      newvar = (
        <div className="">
          {/* <div align="center" ><h2 style={{color:charttext}}>Event scatter chart for S&R AI daily bars 3M Event Of {this.props.boAvailable[0][0]['date']}</h2></div>  */}
          {
            <AnyChart
              width="100%"
              height={400}
              instance={chart5}
              id="bubbleChartBOSNR2"
            />
          }
        </div>
      );
...
const mapStateToProps = (state) => {
  return {
    eventsData: state.Exchange.eventsSNR2Data,
    boAvailable: Object.keys(state.Exchange.priceData)
      .filter((priceDataIndex) => priceDataIndex == "boAvailableSNR2")
      .map((priceDataIndex) => {
