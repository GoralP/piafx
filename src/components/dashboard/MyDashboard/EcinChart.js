...
import {clearData,getmoveDate,ecinclickclear,savestatus,ecinclearData} from "appRedux/actions/Exchange";
        ...
        if(this.props.ecinData['ecin_data'] || ecindataArray.length > 0){
            ...
            newvar = <div>
             {ecindataArray.map((key,item)=>{
                ...
                firstSeriesData.name(title);

                if(code == 'IRSPREAD')
                {
                  // console.log('asplEcin',key['ecin_data1']);
                  var dataSet1 = anychart.data.table('date');
                  dataSet1.addData(key['ecin_data1']);
                  var mapping1 = dataSet1.mapAs({'value': 'value'});
                  var firstSeriesData1 = firstPlot.line(mapping1);
                  firstSeriesData1.name('IREURO');
                  firstSeriesData.name('IRUSA');
                  firstSeriesData.stroke('red');
                }
                ...
                if(unit == 'Rate')
                {
                  ...
                  if(code == 'IRSPREAD')
                  {
                    firstSeriesData.tooltip().format('{%seriesName}: {%value} %');
                    firstSeriesData1.tooltip().format('{%seriesName}: {%value} %');
                  }
                }
                ...
                // {
                  ...
                  {
                    // console.log('asplecin',this.props.tableData);
                    var markers = []
                ...
                //   );
                  ...
                  var graphclick = i;
                return(
                    <div style={{display:'inline-block'}}   className={this.state[switchstate] && this.props.moveDate.moveDate!='' ? "container_white cust_price_chart cust_chart_box" : " cust_price_chart cust_chart_box"}>
                      <div className="ecin_top_div_country">
                        <p>
                          <img src={flag} class="ecin_flag" ></img>
                          <p class="ecin_country" >{country}</p>
                        </p>
                        <p class="ecin_title" >{title}</p>
...
export default connect(mapStateToProps,{ecinclearData,clearData,getmoveDate,ecinclickclear,savestatus})(EcinChart);
