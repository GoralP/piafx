...
import { getEvents, getFilterState } from "appRedux/actions/Exchange";
...
class BOEvents extends Component {
  constructor(props) {
    super(props);
  }
  state = {
  ...
  handleBOEventSubmit = () => {
    if (
      this.props.boAvailable &&
      this.props.boAvailable.length > 0 &&
      this.props.boAvailable[0].length > 0
    ) {
      this.setState({ filterStats: false });
      let dir = this.props.boAvailable[0][0]["3M"];
  ...
  render() {
    ...
    if (
      ...
      if (
        ...
        const date1 = new Date(this.props.boAvailable[0][0]["date"]);
    ...
    if (
      ...
      // let markervalue = this.props.eventsData.bubble.find(x => x.date === this.props.boAvailable[0][0]['date'])
      ...
      // console.log('asplpattern1',markervalue,this.props.boAvailable[0][0]['date'],this.props.eventsData.bubble);
      ...
      //       console.log(this.getData("co"));
      //       if (
      //         temp.props.eventsData.BD > this.getData("co") &&
      //         this.getData("co") < 0
      //       ) {
      //         if (this.getData("co") < 0) {
      //           return purple + " 0.75";
      ...
      newvar = (
        <div className=" ">
          {/* <div align="center" ><h2 style={{color:charttext}}>Event scatter chart for S&R ML daily bars 3M Event Of {this.props.boAvailable[0][0]['date']}</h2></div>  */}
          {
            <AnyChart
              width="100%"
              height={400}
    ...
    return (
      <div style={{ width: "100%" }}>
        {/* {(this.props.boAvailable && this.props.boAvailable.length>0 && this.props.boAvailable[0].length>0)? */}
        {visible ? (
          <>
            <div align="center">
              <h2 style={{ color: charttext }}>
                S&R ML Daily Bars 3M{" "}
                {this.props.boAvailable[0][0]["3M"] === "down"
                  ? "Bearish"
                  : "Bullish"}{" "}
                Event Of {this.props.boAvailable[0][0]["date"]}
              </h2>
            </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    eventsData: state.Exchange.eventsData,
