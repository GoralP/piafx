...
class SNRExt_M extends Component {
    static contextTypes = {
    ...
    render() {
        ...
        return (
            <div style={{"width":"100%"}} className="cust_bullet_chart cust_ecin_div cust-tab-font ">
              {/* <span className="cust_desciption cust-hass-font">S&R AI Monthly Ext. Breakout</span> */}
...
const mapStateToProps =  state => {
    ...
    return {
        snrExt: Object.keys(state.Exchange.snrbreakoutdata).filter(priceDataIndex => priceDataIndex == 'snrExt_M').map((priceDataIndex)=>{
            return state.Exchange.snrbreakoutdata[priceDataIndex];
        }),
    }
  };
  export default connect(mapStateToProps, {getmoveDate,changeScroller,addClicked,tableData})(SNRExt_M);
