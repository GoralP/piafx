...
const selectmekkodatafromapiRequest = async (mekkodata) =>
  await axios
    ...
    .then((result) => result)
    .catch((error) => error);
const getCorrellsDatafromapiRequest = async (corrdata) =>
  await axios
    .post(baseURL + "api/v1/get_correll_data", {
      ticker_code: corrdata["ticker_code"],
      days: corrdata["days"],
    })
    .then((result) => result)
    .catch((error) => error);
...
function* getCorrellsDatafromapi({ payload }) {
  try {
    const getCorrelation = yield call(getCorrellsDatafromapiRequest, payload);
    if (getCorrelation.status) {
      yield put(getCorrellDataComplete(getCorrelation.data));
    }
  } catch (error) {
    console.log("Error");
  }
...
export function* getCorrellsData() {
  yield takeEvery(GET_CORRELL_DATA, getCorrellsDatafromapi);
}
